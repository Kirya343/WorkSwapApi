...
function loadConversations() {
    if (stompClient && stompClient.connected) {
        ...
        connect(function () {
            stompClient.send("/app/getConversations", {"locale": userLocale}, JSON.stringify({}));
            console.log("Отправили запрос на диалоги")
        });
    }
}
...
function addConversationToList(conversation) {
    ...
    console.log("addConversationToList Получил диалог")
    ...
    console.log("addConversationToList обработал диалог")
}
...
function updateSingleConversation(conversation) {
    ...
    if (!dialogItem) {
        ...
        dialogItem.classList.add("appearance-top-animation");
        setTimeout(() => {
            dialogItem.classList.add("visible");
        }, 10);
...
function showMessage(message) {
    ...
    messageElement.classList.add("appearance-top-animation")
    setTimeout(() => {
